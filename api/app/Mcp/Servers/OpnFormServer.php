...
#[Name('OpnForm')]
#[Version('1.0.0')]
#[Instructions('Default every request to create a new form to the guest draft workflow unless the user explicitly asks to save it in their connected OpnForm account or a workspace. A natural request such as "create a contact form" needs no login or preview wording: validate, then create_form_draft, which renders the interactive preview. Every patch_form_draft renders the updated preview too. Follow the catalog authoring guidelines: human labels, useful placeholders, appropriate fields/layout, a contextual submit label, and a useful completion message. Correct relevant quality_warnings while preserving intentional choices. For a long answer, use type text with multi_lines true; textarea is not a valid field type. If validation fails, correct and revalidate the definition before creating a draft. Never ask the user to connect when guest draft tools can satisfy the request. OAuth is required only for connected-account operations. Enabling or selecting the plugin is not OAuth authentication. Use preview_form_draft only to re-render an existing draft. After each guest create or patch, briefly summarize the result and ask whether the user wants another change or wants to save the form to their OpnForm account. Never call a temporary guest draft saved. Once an account-owned form is saved as an unpublished draft, say so and ask whether the user wants it published; asking is not confirmation, so publish only after an explicit affirmative response. If an account-scoped tool challenges, ask the user to authenticate the OpnForm MCP server in the host; do not repeatedly retry from the guest workflow. In local Codex, start a new conversation after OAuth so the MCP client loads the stored credential. If guest draft tools are unexpectedly missing, do not substitute an authenticated tool unless the user requested account persistence; ask the user to start a new conversation with OpnForm selected before the first message. Use OpnForm MCP tools directly. Never invoke Codex or ChatGPT recursively, run `codex exec`, use shell or raw HTTP, inspect repositories or caches, or switch connectors. Read the schema and field catalog again before changing presentation, fields, layout, or media, then validate before saving. Focused presentation creates one step per block automatically: use full-width blocks, no page breaks or standalone media blocks, and attach optional media through the block image property. Use only durable public HTTPS asset URLs, never localhost or temporary tunnel URLs.')]
